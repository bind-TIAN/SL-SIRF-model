...
class parameters():
    def __init__(self, args):
        ...
        self.particle_number = args.particle_number
...
def sample_hyperparameters():
    while True:
        yield {
            ...
            "particle_number": np.random.choice([10, 20, 30, 40]).item(),
        }
...
def write_to_file(file, args):
    file.write(
        "rnn_size: " + str(args.rnn_size) + " learning_schedule: " + str(args.learning_schedule) + " grad_clip: " + str(
            args.grad_clip) + " learning_rate: " + str(args.learning_rate) + " decay_rate: " + str(
            args.decay_rate) + " dropout: " + str(args.dropout) + " embedding_size: " + str(
            args.embedding_size) + " neighborhood_size: " + str(args.neighborhood_size) + " grid_size: " + str(
            args.grid_size) + " particle_number: " + str() + '\n')


def print_to_screen(args):
    print("rnn_size: " + str(args.rnn_size), " learning_schedule: ", str(args.learning_schedule), " grad_clip: ",
          str(args.grad_clip), " learning_rate: ", str(args.learning_rate), " decay_rate: ", str(args.decay_rate),
          " dropout: ", str(args.dropout), " embedding_size: ", str(args.embedding_size), " neighborhood_size: ",
          str(args.neighborhood_size), " grid_size: ", str(args.grid_size), " particle_number: ",
          str(args.particle_number))
...
def main():
    ...
    parser.add_argument('--best_n', type=int, default=5,
    ...
    dataloader_t = DataLoader(f_prefix, args.batch_size, args.sequence_length, num_of_validation=args.num_validation,
                              forcePreProcess=True, infer=True)
    dataloader_v = DataLoader(f_prefix, 1, args.sequence_length, num_of_validation=args.num_validation,
                              forcePreProcess=True, infer=True)

    for hyperparams in itertools.islice(sample_hyperparameters(), args.num_samples):
        args = parameters(parser)
        # randomly sample a parameter set, here we pop the decay_rate, but there is no where to use decay_rate in next codes.
        # here, we add particle_number into this file.
        ...
        args.particle_number = hyperparams.pop("particle_number")
        ...
        # Training
        for epoch in range(args.num_epochs):  # the default epoch is 3
                ...
                for sequence in range(dataloader_t.batch_size):
                    ...
                    if args.method == 3:  # vanilla lstm
                        ...
                        outputs, _, _ = net(x_seq, grid_seq, hidden_states, cell_states, PedsList_seq, numPedsList_seq,
                                            dataloader_t, lookup_seq, x_seq_direction, x_seq_velocity)
                    # Compute loss
            ...
            for sequence in range(dataloader_v.batch_size):
                ...
                if args.method == 3:  # vanilla lstm
                    ...
                    ret_x_seq, loss, denom_loss = sample_validation_data(x_seq, PedsList_seq, grid_seq, args, net,
                                                                         lookup_seq, numPedsList_seq, dataloader_v,
                                                                         x_seq_direction, x_seq_velocity)
