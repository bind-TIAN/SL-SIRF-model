...
def plot_trajectories(true_trajs, pred_trajs, nodesPresent, look_up, frames, name, plot_directory, min_threshold,
    ...
    look_up : Lookup table converting ped ids to array indices

    plot_directory : directory that plots will be stored

    min_threshold : minimum trajectory length to be filtered

    max_ped_ratio : percentage of peds in a frame to be visualized

    target_id : target ped id for this sequence

    style : an array includes max_r, min_r and plot offset (for centering trajectories)
    ...
    true_trajs = true_trajs[1:]  # true_trajs[:]
    ...
    # only consider prediction of target id because of model.
    dict_pred = get_min_max_in_sequence(pred_trajs[:, look_up[target_id], :][:, None, :].numpy())
    ...
    # add target id to at the beginning of the list therefore it will process always
    ...
    for j in shuffled_ped_ids:
        ...
        if not len(true_x) > min_threshold or not len(true_x) > 2:
            # "Ped processing is aborted because its trajectory length in this sequence is smaller than threshold or 1 points"
            continue
        else:
        ...
        # extract non-nan values and scale it according to min and max overall values in this frame
        ...
        # will be used for a video for each frame
...
def get_overall_max_min(dict_true, dict_pred):
    # get the maximum and minimum value of {true,predict}
...
def main():
    ...
    parser.add_argument('--min_traj', type=int, default=3,
                        help='Min. threshold of number of frame to be removed from a sequence')
    # percentage of peds will be taken for each frame
    parser.add_argument('--max_ped_ratio', type=float, default=0.8,  # 0.8
    ...
    # plot_file_name_2 = [plot_file_name[1], plot_file_name[4], plot_file_name[5], plot_file_name[9], plot_file_name[13],
    #                    plot_file_name[15]]
    # num_of_data = np.clip(args.num_of_data + 6, 0, len(plot_file_name))
    # plot_file_name = random.sample(plot_file_name, num_of_data)
    for file_index in range(len(plot_file_name)):  # len(plot_file_name)(3,4;16,17)
        file_name = plot_file_name[file_index]
        # file_name = plot_file_name_2[file_index]
        ...
        # save_video(figure_save_directory, video_save_directory, plot_file_name_2[file_index], args.frame)
