...
def main():
    ...
    parser.add_argument('--method', type=int, default=1,
                        help='Method of lstm will be used (1 = social lstm)')
    ...
    saved_args.use_cuda = False  # this code is not needed. if the model file contain use_cuda, it is time to use this code.
    ...
    for iteration in range(sample_args.iteration):
        ...
        if os.path.isfile(checkpoint_path):
            ...
            model_epoch = checkpoint['epoch']  #read epoch
            net.load_state_dict(checkpoint['state_dict'])  # read parameters of saved models
        ...
        for batch in range(dataloader.num_batches):
            ...
            if dataset_pointer_ins is not dataloader.dataset_pointer:
                ...
                results = []
            # submission.append(
            #    submission_preprocess(dataloader, ret_x_seq.data[sample_args.obs_length:, lookup_seq[target_id],
...
def sample(x_seq, Pedlist, args, net, true_x_seq, true_Pedlist, saved_args, dimensions, dataloader, look_up,
    ...
    with torch.no_grad():
        ...
        for tstep in range(args.obs_length - 1):
            ...
            next_x, next_y, no_use = sample_gaussian_2d(1, mux.data, muy.data, sx.data, sy.data, corr.data,
                                                        true_Pedlist[tstep],
                                                        look_up)
        ...
        # And we have to admit that x_seq_direction_[:args.obs_length] and x_seq_velocity[:args.obs_length] all zero!
        # Thus, we say that this method must include redundancy and it cannot be solved easily!!
